...
export default function DataSearchFilter({ 
  ...
  const handleAddFilter = () => {
    if (!selectedColumn || (!searchValue.trim() && !['is_null', 'is_not_null'].includes(operator))) {
      console.log('❌ Cannot add filter - missing data')
      return
    }

    console.log('➕ Adding filter:', { selectedColumn, operator, searchValue })
  ...
  const handleSearch = () => {
    ...
    console.log('🔍 Search triggered with filters:', activeFilters)
    ...
    <div style={{ 
      ...
      {!tableSchema || tableSchema.length === 0 ? (
        <div style={{ 
          ...
          ⚠️ No table schema available for filtering
        </div>
      ) : (
        <div>
